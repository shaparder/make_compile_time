...
typedef struct buffer
{
        ...
        semaphore_t  full;             /* keep track of the number of full spots */
        semaphore_t  empty;            /* keep track of the number of empty spots */
...
int main(int argc, const char* argv[])
{
  ...
  volatile int empty = NBUF;
  volatile int full = 0;
  seminit(&(buf.full), full);
  seminit(&(buf.empty), empty);
  printf("\n %d", buf.empty.nb);
  printf("\n %d", buf.empty.nb);
  buf.mutex = 0;
